...
print('os.environ["CUDA_VISIBLE_DEVICES"] = "2"')
import torch
import numpy as np
...
# n_layers = 6 * 1 # number of Encoder of Decoder Layer（Block的个数）
...
# # FeedForward dimension (两次线性层中的隐藏层 512->2048->512，线性层是用来做特征提取的），当然最后会再接一个projection层
# d_ff = 2048
# d_k = d_v = 252*2  # dimension of K(=Q), V（Q和K的维度需要相同，这里为了方便让K=V）
# n_layers = 16 # number of Encoder of Decoder Layer（Block的个数）
# n_heads = 8 * 3  # number of heads in Multi-Head Attention（有几套头）
...
X = np.linspace(-1.1, 1.1, num=N_sample_point)
X= X.reshape(N_sample_point,1)
# X = abs(X)
x1 = X[:,0]
# y = np.exp(x1) * np.sin(x1)
y = np.sin(x1*2) * np.cos(x1) + 1
# y = np.sin(x1**2) + x1
# l = ["+", "sin", "var_x1", "sin", "var_x1"]
# y = all_farward(l,x1)
